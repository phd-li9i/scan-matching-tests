  ...
  const double& pd_threshold)
{
  ...
  if (method == 3)
  {
    ...
    if (vendalia_method == 1)
      pick_num_surr = 1; //pow(2,7) / pow(2,magnification_size);
    if (vendalia_method == 2)
      pick_num_surr = 4;
...
/*******************************************************************************
*/
std::pair<double,double> Rotation::skg1(
  const std::vector< double >& real_scan,
  const std::tuple<double,double,double>& virtual_pose,
  const std::vector< std::pair<double,double> >& map,
  double* dt)
{
  ...
  Utils::points2scan(intersections, virtual_pose, &virtual_scan);
  ...
  //dumpMap(map);
  //dumpMaps(real_scan_points, virtual_scan_points);
  //dumpScan(real_scan, zero_pose, virtual_scan, zero_pose);
  ...
  std::vector<double> R1 =
    DFTUtils::getFirstDFTCoefficient(real_scan);

  std::vector<double> V1 =
    DFTUtils::getFirstDFTCoefficient(virtual_scan);
  ...
  double dx = d.first;
  double dy = d.second;
  double d_norm = Utils::norm(d);

  std::pair<double,double> real_centroid = Find::centroid(real_scan_points);
  std::pair<double,double> virtual_centroid = Find::centroid(virtual_scan_points);

  double ex = real_centroid.first - virtual_centroid.first;
  double ey = real_centroid.second - virtual_centroid.second;
  double e_norm = sqrt(ex*ex + ey*ey);

  double nomin;
  double denom;

  int calc_fashion = 3;
  if (calc_fashion == 1) // OK under rotational only, immediate convergence
  {
    nomin = V1[0]*R1[1] - V1[1]*R1[0];
    denom = V1[0]*R1[0] + V1[1]*R1[1];
  }

  double N = 1.0;
  if (calc_fashion == 2) // not ok
  {
    nomin = V1[0]*(N*(dy-ey)-R1[1]) + V1[1]*(R1[0]+N*(dx-ex));
    denom = V1[0]*(N*(dx-ex)+R1[0]) + V1[1]*(R1[1]-N*(dy-ey));
  }

  if (calc_fashion == 3) // ok, swift convergence
  {
    nomin = R1[1]*(V1[0] + N*(ex-dx)) + R1[0]*(-V1[1] + N*(ey-dy));
    denom = R1[0]*(V1[0] + N*(ex-dx)) + R1[1]*(+V1[1] - N*(ey-dy));
  }

  if (calc_fashion == 4) // not ok
  {
    ex = 0;
    ey = 0;
    nomin = R1[1]*(V1[0] + N*(ex-dx)) + R1[0]*(-V1[1] + N*(ey-dy));
  ...
  *dt = atan(nomin / denom);
  ...
  printf("output pose (%f,%f,%f) [skg1]\n",
    std::get<0>(virtual_pose),
    std::get<1>(virtual_pose),
    std::get<2>(virtual_pose)+*dt);
#endif

  return std::make_pair(d_norm,e_norm);
}


/*******************************************************************************
*/
std::pair<double,double> Rotation::skg2(
  const std::vector< double >& real_scan_in,
  const std::vector< std::pair<double,double> >& real_scan_points,
  const std::tuple<double,double,double>& virtual_pose_in,
  const std::vector< std::pair<double,double> >& map,
  std::tuple<double,double,double>* result_pose,
  const bool& use_centroids,
  const int& magnification_size)
{
  std::tuple<double,double,double> zero_pose;
  std::get<0>(zero_pose) = 0.0;
  std::get<1>(zero_pose) = 0.0;
  ...
  *result_pose = virtual_pose_in;

#if defined (PRINTS)
  printf("input pose  (%f,%f,%f) [closedFormRotation]\n",
    std::get<0>(*result_pose),
    std::get<1>(*result_pose),
    std::get<2>(*result_pose));
#endif
  ...
  std::vector<double> virtual_scan;
  std::vector< std::pair<double,double> > intersections =
    X::find(virtual_pose_in, map, magnification_size);
  Utils::points2scan(intersections, virtual_pose_in, &virtual_scan);

  std::vector< std::pair<double,double> > virtual_scan_points;
  Utils::scan2points(virtual_scan, zero_pose, &virtual_scan_points);

  std::vector<double> real_scan;
  Utils::points2scan(real_scan_points, zero_pose, &real_scan);
  Utils::points2scan(virtual_scan_points, zero_pose, &virtual_scan);
  ...
  /*
     std::vector<double> real_scan_new;
     std::vector<double> virtual_scan_new;
     double thres = 0.5;
     for (int i = 0; i < real_scan.size()-1; i++)
     {
     if (std::abs(real_scan[i+1]-real_scan[i]) > thres)
     real_scan_new.push_back(max(real_scan[i], real_scan[i+1]));
     else
     real_scan_new.push_back(real_scan[i]);

     if (std::abs(virtual_scan[i+1]-virtual_scan[i]) > thres)
     virtual_scan_new.push_back(max(virtual_scan[i], virtual_scan[i+1]));
     else
     virtual_scan_new.push_back(virtual_scan[i]);
     }

     real_scan = real_scan_new;
     virtual_scan = virtual_scan_new;
     */

  /*
     std::vector<double> real_scan_new = real_scan;
     std::vector<double> virtual_scan_new = virtual_scan;
     double thres = 1.2;
     for (int i = 0; i < real_scan.size()-1; i++)
     {
     if (std::abs(real_scan[i+1]-real_scan[i]) > thres)
     real_scan_new[i]= min(real_scan[i], real_scan[i+1]);

     if (std::abs(virtual_scan[i+1]-virtual_scan[i]) > thres)
     virtual_scan_new[i] = min(virtual_scan[i], virtual_scan[i+1]);
     }
  ...
  double nomin = R1[1]*(V1[0] + (ex-dx)) + R1[0]*(-V1[1] + (ey-dy));
  double denom = R1[0]*(V1[0] + (ex-dx)) + R1[1]*(+V1[1] - (ey-dy));

  double c = atan(nomin / denom);
  Utils::wrapAngle(&c);
  std::get<2>(*result_pose) += c;
  Utils::wrapAngle(&std::get<2>(*result_pose));

#if defined (PRINTS)
  printf("output pose (%f,%f,%f) [closedFormRotation]\n",
    std::get<0>(*result_pose),
    std::get<1>(*result_pose),
    std::get<2>(*result_pose));
#endif
